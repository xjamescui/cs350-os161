...
int main() {
	...
	TEST_EQUAL(saved_errno, EBADF,
			"Expected EBADF when writing to closed file f1");
...
#ifdef USING_ERR_CODES
	TEST_EQUAL(saved_errno, EBADF,
			"Expected EBADF when reading from closed file f2");
#endif /* USING_ERR_CODES */
...
#ifdef USING_ERR_CODES
	TEST_EQUAL(saved_errno, EFAULT,
			"Expected EFAULT for invalid address for filename");
#endif /* USING_ERR_CODES */
